...
const SHARED_CHANNEL_DATA_KEY: InjectionKey<InjectedChannelData> =
  Symbol("channel-data");
 ...
 */
export function provideChannelData(workspaceId: ComputedRef<string>) {
  const paginatedMessagesData: Ref<{ messages: PaginatedMessageList | null }> =
    ref({
      messages: null,
    });

  const data = ref<ChannelData | null>();
  ...
  const setPaginatedMessagesData = (data: {
    messages: PaginatedMessageList | null;
  }) => (paginatedMessagesData.value = data);

  const setChannelData = (channelData: ChannelData | null) =>
  ...
  const loadMore = async (options?: {
    ...
    if (loadingMore.value || !canLoadMore.value || !data.value?._id) return;
    loadingMore.value = true;

    return $fetch(`/api/channel/${data.value?._id}/loadMore`, {
      method: "GET",
  ...
  const addMessage = async (
    ...
    _conversation_id?: Id<"dm_conversations">,
    options?: {
      onSuccess?: () => void;
      onError?: () => void;
    }
  ...
  const injectedData = {
    ...
    toggleReaction,
    deleteMessage,
  } as unknown as InjectedChannelData;

  return {
    data: injectedData,
    provide: () => provide(SHARED_CHANNEL_DATA_KEY, injectedData),
  };
}

export function injectChannelData() {
  const injectedData = inject(SHARED_CHANNEL_DATA_KEY);

  if (!injectedData) {
    throw new Error(
      "No shared channel data found. Make sure the parent component has called provideChannelData first"
    );
  }

  return injectedData as InjectedChannelData;
}
